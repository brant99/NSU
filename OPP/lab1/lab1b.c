...
int main(int argc,char *argv[]){
    ...
    b=calloc(sizeCol,sizeof(double));
    x=calloc(sizeCol,sizeof(double));
    ...
    do{
      ...
      for(i=0;i<amount_proc;i++){
        if(n%amount_proc==i)
          recSize--;
        MPI_Sendrecv(x,sizeCol,MPI_DOUBLE,i,process_id,&u[shRecCol],recSize,MPI_DOUBLE,i,i,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
        shRecCol+=recSize;
      }
      MPI_Barrier(MPI_COMM_WORLD);
      mult(piece,a,u,n,sizeCol);
      for(i=0;i<sizeCol;i++){
        piece[i]-=b[i];
      }
      recSize=n/amount_proc+1;
